...
def calculate_automatic_weights(matrix):
    def calculate_entropy(column):
        somatorio = 0
        for x in column:
            somatorio += (x * np.log(x + 0.00001))
        
        entropy = ((1/np.log( len(column) ) ) * somatorio) * -1
        return entropy

    def calculate_diversity(column):
        entropy = calculate_entropy(column)
        diversity = 1 - entropy

        return diversity

    if np.any(matrix.T[0] < 0):
        matrix.T[0] = (matrix.T[0] + 1) / 2
    
    diversities = [calculate_diversity(column) for column in matrix.T]
    total_diversity = np.sum(diversities)
    weights = diversities / total_diversity
    return weights
...
class Topsis():
    ...
    def step_2(self):
        ...
        if self.is_entropy == True:
            self.weight_matrix = calculate_automatic_weights(self.normalized_decision)
            print(f'Entropy weights -> {self.weight_matrix}')
    ...
    def calc(self):
        ...
        #print("**************************")
        #print(f"The Best Option: {self.evaluation_matrix[self.rank_to_best_similarity()[0] - 1]}")
