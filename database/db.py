...
class Database:
    ...
    @torch.no_grad()
    def search(self, x, nrt_neigh=5, retrieve_class='true'):
        t_model = time.time()
        ...
        if retrieve_class == 'true':
            distance, labels = self.index_labeled.search(out, nrt_neigh)
            ...
            distance, labels = self.index_unlabeled.search(out, nrt_neigh)
            labels = [l for l in list(labels[0]) if l != -1]
            names = []
            print(distance)
            for l in labels:
            ...
            distance_l, labels_l = self.index_labeled.search(out, nrt_neigh)
            distance_u, labels_u = self.index_unlabeled.search(out, nrt_neigh)
...
if __name__ == "__main__":
    ...
    model = models.Model(num_features=4096, model=args.extractor, name=args.weights)
