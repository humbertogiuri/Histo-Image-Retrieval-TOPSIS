...
class Model(nn.Module):
    def __init__(self, model='densenet', eval=True, batch_size=32, num_features=128,
                 name='weights', use_dr=True, device='cuda:0', freeze=False, retrain=""):
        ...
        if model == 'densenet':
            ...
            print('mobile')
            self.forward_function = self.forward_conv
            self.conv_net = models.mobilenet_v2(pretrained=True).to(device=device)
        ...
        if use_dr and model != 'transformer':
            ...
            if model == 'densenet':
                ...
                self.conv_net.classifier = nn.Linear(1280, num_features).to(device=device)
            elif model == 'transformer':
        ...
        if eval == True:
            ...
            if retrain != "":
                print(f"Retreinando: {retrain}")
                self.load_state_dict(torch.load(retrain))
            self.train()
            self.eval = False
...
if __name__ == "__main__":
    ...
    m = Model(model=args.model, eval=False, batch_size=args.batch_size,
              ...
              use_dr=args.dr_model, device=device, freeze=args.freeze, retrain=args.retrain)
